...
{
    // A negative input batch is the explicit shared-input-Hadamard mode used
    // by the vLLM adapter.  It is safe only when all selected matrices use an
    // identical suh vector.  In that case one grid slice transforms the
    // broadcast activation and every expert consumes the same result.
    bool pretransformed_input = bszm_in < -MAX_INDICES;
    bool shared_input_had = bszm_in < 0;
    int input_batch = pretransformed_input
        ? -bszm_in - MAX_INDICES
        : (shared_input_had ? -bszm_in : bszm_in);
    int bszm = MAX(input_batch, bszm_out);
    auto grid = cg::this_grid();
    ...
    for (int i = 0; i < bszm; i += gridDim.z)
    {
        ...
        if (
            B && !pretransformed_input
            && (!shared_input_had || (i == 0 && blockIdx.z == 0))
        )
        {
            int total_warps = size_m * size_k / 128;
            int warps_grid = gridDim.x * blockDim.x / 32;
            ...
            const half* A_ = input_batch == 1 ? A : A + j * size_m * size_k;
            half* A_had_ = shared_input_had
                ? A_had
                : A_had + j * size_m * size_k;
        ...
        int size_m_ = size_m;
        half* A_ = shared_input_had
            && (!pretransformed_input || input_batch == 1)
            ? A_had
            : A_had + j * size_m * size_k;
        void* C_;
