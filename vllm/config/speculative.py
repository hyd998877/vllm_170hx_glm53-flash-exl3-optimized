...
class SpeculativeConfig:
    ...
    """Optional per-layer online quantization overrides for the draft model.

    This is intentionally scoped to the speculative/draft model.  In
    particular, DFlash's fused context-KV projection reads the raw QKV weight
    tensor directly, so callers can leave ``q_proj``, ``k_proj`` and
    ``v_proj`` unquantized while quantizing the remaining draft linears.
    """
    ...
    def __post_init__(self):
        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            if self.model is not None:
                ...
                # DFlash heads run in full on the last target PP worker. They
                # must not inherit the target pipeline split because the head
                # itself is not partitioned across those workers.
