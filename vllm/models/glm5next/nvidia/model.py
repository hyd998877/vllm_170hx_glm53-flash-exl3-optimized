...
class Glm5NextModel(nn.Module, EagleModelMixin):
    ...
    def make_empty_intermediate_tensors(
        ...
        # mHC keeps four residual streams between decoder layers. At a PP
        # boundary the sending stage materializes its deferred hc_post, so the
        # receive buffer must preserve that extra stream dimension.
        ...
        if self.aux_hidden_state_layers:
            tensors["aux_hidden_states"] = torch.zeros(
                batch_size,
                len(self.aux_hidden_state_layers),
                self.config.hidden_size,
                dtype=dtype,
                device=device,
            )
    ...
    def _materialize_aux_hidden_state(
        ...
        post_layer: Glm5NextDecoderLayer | None,
    ) -> torch.Tensor:
        """Return the target feature consumed by the GLM DFlash checkpoint.

        The official GLM DFlash capture path taps immediately before the next
        decoder layer, where the preceding layer's mHC post-mix has already
        been materialized.  This implementation defers that post-mix so it can
        be fused with the next layer's pre-mix.  Materialize the same completed
        layer output on this side path before contracting its residual streams.
        """
        if residual is not None:
            assert post is not None and comb is not None and post_layer is not None
            return hc_contract(
                post_layer.hc_post(hidden_states, residual, post, comb),
                post_layer.n,
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if get_pp_group().is_first_rank:
            ...
            comb = None
            if self.aux_hidden_state_layers:
                aux_hidden_states = hidden_states.new_zeros(
            ...
            # PP boundaries transmit a materialized mHC residual stream. The
            # first local layer begins a fresh deferred post/pre chain.
            residual = None
            post = None
            comb = None
            if self.aux_hidden_state_layers:
                aux_hidden_states = intermediate_tensors.tensors.get(
                    "aux_hidden_states"
                )
        ...
        for local_idx, layer in enumerate(self._active_layers):
            layer_idx = self.start_layer + local_idx
            hidden_states, residual, post, comb = layer(
                positions, hidden_states, residual, post, comb
            )
            # DFlash layer ids denote the target block output (the generic
            # Eagle3 config converts them to one-based ids).  mHC defers this
            # output's post-mix until the next block, so materialize it on a
            # side path while the deferred state is still available.
            tap_layer = layer_idx + 1
            if (
                aux_hidden_states is not None
                and tap_layer in self.aux_hidden_state_layers
            ):
                slot = self.aux_hidden_state_layers.index(tap_layer)
                aux_hidden_states[:, slot] = self._materialize_aux_hidden_state(
                    hidden_states, residual, post, comb, layer
                )

        if not get_pp_group().is_last_rank:
            # There is no fused post/pre across PP ranks. Finish the last local
            # layer before sending the materialized residual streams.
            if post is not None:
            ...
            if aux_hidden_states is not None:
                tensors["aux_hidden_states"] = aux_hidden_states
            return IntermediateTensors(tensors)
        ...
        if aux_hidden_states is not None:
            return hidden_states, [
                aux_hidden_states[:, i]
                for i in range(aux_hidden_states.shape[1])
            ]
        return hidden_states
...
def _try_load_fp8_indexer_wk(name, tensor, buf, params_dict, loaded_params):
    ...
    # In PP workers, checkpoint shards for other stages are still iterated;
    # their fused indexer parameter is intentionally absent on this rank.
...
def _try_load_fp8_attn_proj(
    ...
    if target_s in params_dict:
        return False
    # Under pipeline parallelism each worker only materializes its own layer
    # slice.  The checkpoint iterator still visits all layers, so a special
    # FP8 dequant path must skip projections whose destination is a missing PP
    # layer instead of indexing ``params_dict`` and raising KeyError.
