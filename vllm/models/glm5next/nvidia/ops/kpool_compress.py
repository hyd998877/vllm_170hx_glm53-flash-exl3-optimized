...
def _f32_to_e4m3fn_u8(x):
    """Software e4m3fn RNE encoder for CUDA devices older than SM89."""
    ...
    keep += round_up.to(tl.uint32)
    normal = (((exp + 6) << 3).to(tl.uint32) + keep).to(tl.uint32)
    val = tl.where(exp >= -6, normal, keep)
...
def kpool_seed_tail_cache(
    ...
    _kpool_tail_seed_kernel[(n,)](
        ...
        tail_kv_cache,
        n,
        NUM_TAIL_BLOCKS=tail_kv_cache.shape[0],
