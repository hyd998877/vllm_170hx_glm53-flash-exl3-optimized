    ...
    *content* (softmax-weighted sum vs keep-every-Nth) is computed by the
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        if self._index_kpool == 1:
            # Keep a dense 64-token physical page so a large manager block
            # (chosen to match the KDA/Mamba state page) can be split into
            # kernel blocks. ``storage_block_size`` also marks this kpool=1
            # spec as an indexer for the GLM-5 side-by-side cache allocator.
            return replace(spec, storage_block_size=64)
        assert (
...
class Indexer(nn.Module):
    def __init__(
        ...
        # Keep the checkpoint's pooling factor on Ampere as well.  The SM80
        # path uses the portable software FP8 encoder and Triton MQA logits;
        # changing this to one entry per token silently changes the trained
        # sparse-indexer representation and loses long-context recall.
