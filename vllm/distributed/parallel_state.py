...
class GroupCoordinator:
    ...
    def __init__(
        ...
        # PP forwards form a chain, while PyTorch's batched P2P API requires
        # all ranks in a process group to enter its first call together.  A
        # four-rank PP group therefore cannot be used directly for the first
        # 0->1, 1->2, and 2->3 transfers.  Create deterministic adjacent
        # two-rank groups for PP tensor-dict traffic; all world ranks still
        # call ``new_group`` in the same order, including non-members.
        self._pp_p2p_device_groups: dict[tuple[int, int], ProcessGroup] = {}
        if group_name == "pp" and any(len(ranks) > 1 for ranks in group_ranks):
            from vllm.distributed.utils import get_distributed_timeout_or_none

            for ranks in group_ranks:
                for left, right in zip(ranks, ranks[1:]):
                    pair = [left, right]
                    device_pair = torch.distributed.new_group(
                        pair,
                        backend=torch_distributed_backend,
                        timeout=get_distributed_timeout_or_none(),
                    )
                    # Non-members may return from new_group before the two
                    # members finish creating their NCCL communicator.  Keep
                    # every world rank at the same group-creation sequence so
                    # later sibling groups (notably PP sampled-token broadcast)
                    # receive the same process-group identity on every rank.
                    torch.distributed.barrier()
                    if self.rank in pair:
                        key = (left, right)
                        self._pp_p2p_device_groups[key] = device_pair
    ...
    def _pp_p2p_group(self, peer: int) -> ProcessGroup:
        """Return the adjacent PP pair group for a peer rank."""
        if not getattr(self, "_pp_p2p_device_groups", None):
            return self.device_group
        current = self.rank
        key = (current, peer) if current < peer else (peer, current)
        return self._pp_p2p_device_groups.get(key, self.device_group)
    ...
    def isend_tensor_dict(
        ...
        group = self._pp_p2p_group(self.ranks[dst])
        metadata_group = self.cpu_group
        ...
        handles: list[Handle] = []
        device_p2p_ops: list[torch.distributed.P2POp] = []
        for key, tensor in zip(tensor_keys, tensor_list):
            ...
            if not tensor.is_cpu:
                ...
                # Gloo/CPU metadata tensors are kept on the existing path.
                # They are not involved in the NCCL PP communicator ordering.
                handles.append(
                    torch.distributed.isend(
        ...
        if device_p2p_ops:
            handles.extend(torch.distributed.batch_isend_irecv(device_p2p_ops))
        return handles
    ...
    def isend_tensor_dict_static(
        ...
        """Send tensors without transmitting shape or dtype metadata."""
        if self.world_size <= 1:
            return []
        if self.use_cpu_custom_send_recv:
            raise RuntimeError(
                "Static tensor-dict communication does not support the custom "
                "CPU send/recv backend"
            )
        if dst is None:
            dst = (self.rank_in_group + 1) % self.world_size
        assert dst < self.world_size, f"Invalid dst rank ({dst})"
        ...
        handles: list[Handle] = []
        device_group = self._pp_p2p_group(self.ranks[dst])
        for key, tensor in tensor_dict.items():
            if not isinstance(tensor, torch.Tensor):
                raise TypeError(
                    "Static tensor-dict communication only accepts tensors; "
                    f"key {key!r} has {type(tensor)}"
                )
            if tensor.numel() == 0:
                continue
            group = self.cpu_group if tensor.is_cpu else device_group
    ...
    def irecv_tensor_dict(
        ...
        group = self._pp_p2p_group(self.ranks[src])
        metadata_group = self.cpu_group
    ...
    def irecv_tensor_dict_static(
        ...
        tensor_dict: dict[str, torch.Tensor],
        src: int | None = None,
    ) -> tuple[dict[str, torch.Tensor], list[Handle], list[Callable[[], None]]]:
        """Receive tensors into caller-provided buffers without metadata."""
        if not torch.distributed.is_initialized() or self.world_size == 1:
            return tensor_dict, [], []
        if self.use_cpu_custom_send_recv:
            raise RuntimeError(
        ...
        handles: list[Handle] = []
        device_group = self._pp_p2p_group(self.ranks[src])
        for key, tensor in tensor_dict.items():
            if not isinstance(tensor, torch.Tensor):
                raise TypeError(
                    "Static tensor-dict communication only accepts tensors; "
    ...
    def destroy(self):
        for pg in getattr(self, "_pp_p2p_device_groups", {}).values():
