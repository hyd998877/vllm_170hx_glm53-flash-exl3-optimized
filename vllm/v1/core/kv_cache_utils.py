...
def _get_kv_cache_groups_glm5_next(
    ...
    Ordinary attention specs can belong to a speculative draft.  They must
    not force the sparse target's non-rectangular MLA/indexer pages through
    generic page-size unification; keep them as separate block-table groups.
    """
    mamba_specs = {
        name: spec
    ...
    extra_specs = {
        ...
        and type(spec) is not MLAAttentionSpec
    }
    if (
        not mamba_specs
        or not mla_specs
        or any(isinstance(spec, HiddenStateCacheSpec) for spec in extra_specs.values())
    ):
        return None
    ...
    def is_indexer(name: str, spec: MLAAttentionSpec) -> bool:
        ...
        # its explicit dense kernel-page width instead.
        return spec.tokens_per_state > 1 or (
            ".indexer.k_cache" in name and spec.storage_block_size is not None
        )

    idx_names = [name for name, spec in mla_specs.items() if is_indexer(name, spec)]
    ...
    mla_names = [name for name in mla_specs if name not in idx_names]
    ...
    if extra_specs:
        ...
        if extra_uniform is None:
            return None
        # Fit the draft pages into distinct target MLA/indexer slots on the PP
        # stage that hosts the local DFlash head. The draft block remains a
        # divisor of the target block, so scheduler/hash conversion stays exact.
        target_block_size = uniform_spec.block_size
        slot_sizes = [mla_page] * len(mla_names) + [next(iter(idx_pages))] * len(
            idx_names
        )
        if vllm_config.parallel_config.pipeline_parallel_size > 1:
            ...
            slot_sizes = [
                ...
                for name, page in zip(
                    [*mla_names, *idx_names],
                    [mla_page] * len(mla_names)
                    + [next(iter(idx_pages))] * len(idx_names),
                )
                if start <= extract_layer_index(name) < end
            ]
        draft_block_size = next(
            (
                candidate
        ...
        if draft_block_size == 0:
            return None
        resized_extra_specs = {
            name: replace(spec, block_size=draft_block_size, page_size_padded=None)
...
def _glm5_next_tensor_layout(
    ...
    A speculative draft can add ordinary attention groups on the final PP
    rank.  Return those as ``extra_groups`` so the GLM target keeps its
    Mamba/MLA and kpool aliasing while the draft receives independent block
    tables in the same physical block pool.
    """
    uniform_groups = [
        group
    ...
    mamba_groups = [
        group
        for group in kv_cache_groups
        if group.layer_names and isinstance(group.kv_cache_spec, MambaSpec)
    ]
    ...
    if attn_group is None or not mamba_groups:
        return None
    if len(uniform_groups) + len(mamba_groups) != sum(
        bool(group.layer_names) for group in kv_cache_groups
    ):
        return None
    ...
    idx_names = [
        ...
        or (
            ".indexer.k_cache" in name
            and mla_inner[name].storage_block_size is not None
        )
    ]
    mla_names = [name for name in attn_group.layer_names if name not in idx_names]
...
def _glm5_extra_slot_layout(
    ...
    layouts: list[dict[str, tuple[int, int, int]]] = []
    for group in extra_groups:
        used = [0] * len(slots)
        assignment: dict[str, tuple[int, int, int]] = {}
        layers = sorted(
            group.layer_names,
            key=lambda name: _get_per_layer_spec(group, name).page_size_bytes,
            reverse=True,
        )
        for layer_name in layers:
            page = _get_per_layer_spec(group, layer_name).page_size_bytes
            candidates = [
                index
                for index, (_, capacity) in enumerate(slots)
                if capacity - used[index] >= page
            ]
            if not candidates:
                return None
            slot_index = min(
                candidates,
                key=lambda index: slots[index][1] - used[index],
            )
            slot_offset, slot_stride = slots[slot_index]
            assignment[layer_name] = (
                slot_offset,
                used[slot_index],
                slot_stride,
            )
            used[slot_index] += page
        layouts.append(assignment)
    return layouts
...
def _get_kv_cache_bytes_per_block(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        # The GLM target uses a layer-major layout with per-layer block strides.
        # Extra draft groups have different page widths, so overlaying them at
        # byte zero can overlap a target block even when the groups own distinct
        # block IDs. Keep their backing regions disjoint.
        return target_width + sum(extra_widths)
...
def get_kv_cache_config_from_groups(
    ...
    if (glm5_layout := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        target_width = len(mla_names) * mla_page + len(idx_names) * idx_page
        extra_slot_layout = _glm5_extra_slot_layout(
            mla_names,
            idx_names,
            mla_page,
            idx_page,
        ...
        bytes_per_block = (
            ...
            else target_width + sum(extra_widths)
        )
        ...
        def add_tensor(
            ...
            kv_cache_tensors.append(
                KVCacheTensor(
                    ...
                    block_stride=block_stride or spec.page_size_bytes,
                    offset=offset,
                )
            )
        ...
        extra_base = target_width * num_blocks
        for group_index, group in enumerate(extra_groups):
            extra_specs = cast(
                UniformTypeKVCacheSpecs, group.kv_cache_spec
            ).kv_cache_specs
            if extra_slot_layout is not None:
                for layer_name in group.layer_names:
                    slot_offset, suboffset, slot_stride = extra_slot_layout[
                        group_index
                    ][layer_name]
                    add_tensor(
                        layer_name,
                        extra_specs[layer_name],
                        slot_offset * num_blocks + suboffset,
                        block_stride=slot_stride,
                    )
            else:
        ...
        logger.info(
            "GLM KV pool ledger: available_bytes=%d, num_blocks=%d, "
            "bytes_per_block=%d, allocated_bytes=%d, target_bytes_per_block=%d, "
            "extra_bytes_per_block=%s, extra_overlaid=%s",
            available_memory,
            num_blocks,
            bytes_per_block,
            size,
            target_width,
            extra_widths,
            extra_slot_layout is not None,
        )
        for group_index, group in enumerate(kv_cache_groups):
            group_spec = group.kv_cache_spec
            logger.info(
                "GLM KV group ledger: group=%d, spec=%s, block_size=%d, "
                "page_bytes=%d, layers=%s",
                group_index,
                type(group_spec).__name__,
        ...
        for tensor in kv_cache_tensors:
            ...
            logger.info(
                "GLM KV tensor ledger: layer=%s, spec=%s, page_bytes=%d, "
                "offset_bytes=%d, block_stride_bytes=%d, layer_stride_bytes=%d, "
                "pool_bytes=%d",
                layer_name,
...
def _pages_fit_storage_slots(page_sizes: list[int], slot_sizes: list[int]) -> bool:
    """Return whether pages fit into per-block slots without overlap."""
    remaining = list(slot_sizes)
    for page_size in sorted(page_sizes, reverse=True):
        candidates = [
            index for index, capacity in enumerate(remaining) if capacity >= page_size
        ]
        if not candidates:
            return False
        slot_index = min(candidates, key=remaining.__getitem__)
        remaining[slot_index] -= page_size
    return True
