...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            # AsyncScheduler may temporarily hold decode requests while a
            # long-prefill cohort finishes its final chunks.  This hook is a
            # no-op for the base scheduler and keeps the policy out of the
            # core scheduling algorithm.
            if self._should_defer_decode_request(request):
                req_index += 1
                continue

            if self._should_defer_prefill_chunk(request):
                req_index += 1
                continue
        ...
        for idx, req in enumerate(itertools.chain(running_reqs, resumed_reqs)):
            ...
            if self.use_pp and not self.scheduler_config.async_scheduling:
                ...
                if self.vllm_config.speculative_config is not None:
                    # Send a bounded tail of *confirmed outputs*.  Slicing
                    # the current compute range is not sufficient for
                    # speculative decoding: after a rejected draft,
                    # num_computed_tokens can point past the output that
                    # non-last PP ranks still need.
                    max_confirmed_per_step = (
                        self.num_spec_tokens + self.num_sampled_tokens_per_step
                    )
                    token_ids = req._output_token_ids[-max_confirmed_per_step:]
                else:
