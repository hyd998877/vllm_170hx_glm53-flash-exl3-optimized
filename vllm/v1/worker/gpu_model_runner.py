...
class GPUModelRunner(
    ...
    def __init__(
        ...
        if self.speculative_config and get_pp_group().is_last_rank:
            ...
            elif "Glm5NextMTPModel" in (
                self.speculative_config.draft_model_config.hf_config.architectures
                or []
            ):
                self.drafter = Glm5NextMTPProposer(
        ...
        # In synchronous PP speculative decoding, non-last stages return
        # intermediate tensors from ``execute_model`` and therefore do not
        # retain an ``ExecuteModelState`` for ``sample_tokens``.  Keep the
        # scheduler metadata needed to commit hybrid recurrent state after
        # the last stage broadcasts the accepted-token count.
        self._last_scheduler_output_for_pp_spec: SchedulerOutput | None = None
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            if not is_last_rank:
                if not req_data.new_token_ids:
                    ...
                    if self.speculative_config is not None:
                        confirmed_token_tail = req_data.new_token_ids[i]
                        # Align from the scheduler's confirmed output count.
                        # In speculative decoding num_computed_tokens may
                        # include a draft that was subsequently rejected, so
                        # it cannot infer how many output IDs this rank misses.
                        num_missing_tokens = max(
                            num_output_tokens - len(req_state.output_token_ids), 0
                        )
                        assert num_missing_tokens <= len(confirmed_token_tail), (
                            "PP scheduler did not return enough confirmed output "
                            f"tokens: missing={num_missing_tokens}, "
                            f"returned={len(confirmed_token_tail)}"
                        )
                        new_token_ids = (
                            confirmed_token_tail[-num_missing_tokens:]
                            if num_missing_tokens
                            else []
                        )
                        if num_missing_tokens == 1:
                            # Avoid slicing list in most common case.
                            req_state.output_token_ids.append(new_token_ids[-1])
                        elif num_missing_tokens > 0:
                            req_state.output_token_ids.extend(new_token_ids)

                        # Only the final PP rank runs the rejection sampler and
                        # therefore updates its hybrid/KDA state with the
                        # number of valid tokens from the verify step. For
                        # synchronous PP the scheduler-confirmed output delta
                        # is that same count; propagate it into every earlier
                        # rank before the next forward selects the state.
                        if req_index is not None:
                            self.input_batch.num_accepted_tokens_cpu[req_index] = max(
                                num_missing_tokens, 1
                            )
                    else:
            ...
            if not is_last_rank:
                ...
                if req_data.new_token_ids and self.speculative_config is not None:
                    # Sync PP: num_computed_tokens may include accepted (or
                    # later rejected) drafts.  The confirmed token boundary is
                    # the prompt plus scheduler-confirmed output count.
                    end_token_index = max(
                        start_token_index,
                        self.input_batch.num_prompt_tokens[req_index]
                        + num_output_tokens,
                    ...
                    end_token_index = max(start_token_index, num_computed_tokens)
                if end_token_index > start_token_index:
                    if new_token_ids:
                        # Add new_token_ids to token_ids_cpu.
                        num_new_tokens = end_token_index - start_token_index
                        assert num_new_tokens <= len(new_token_ids)
    ...
    def _update_states_after_model_execute(
        ...
        pp = get_pp_group()
        if pp.world_size > 1 and not self.use_async_scheduling:
            assert pp.is_last_rank
            # The sampler runs only on the last PP stage.  Hybrid recurrent
            # state postprocessing must nevertheless use the same accepted
            # count on every earlier stage before its next forward.
        ...
        self._postprocess_hybrid_states_after_spec_decode(num_reqs, scheduler_output)

    def _postprocess_hybrid_states_after_spec_decode(
            ...
            if not self.broadcast_pp_output:
                ...
                if not get_pp_group().is_last_rank:
                    ...
                    if (
                        ...
                        self._last_scheduler_output_for_pp_spec = scheduler_output
    ...
    def sample_tokens(
        ...
        if self.execute_model_state is None:
            ...
            if (
                ...
                # Receive the rejection result from the sampling stage and
                # commit the same hybrid recurrent-state candidate locally.
                num_reqs = self.input_batch.num_reqs
                torch.distributed.broadcast(
                    self.num_accepted_tokens.gpu[:num_reqs],
                    src=pp.last_rank,
                ...
                # Non-last ranks do not retain ExecuteModelState, but the
                # scheduler metadata needed by hybrid postprocessing is the
                # same object passed into execute_model on every stage.
                scheduler_output = self._last_scheduler_output_for_pp_spec
                if scheduler_output is not None:
                    self._postprocess_hybrid_states_after_spec_decode(
                        num_reqs, scheduler_output
                    )
                self._last_scheduler_output_for_pp_spec = None
    ...
    def _pp_broadcast_prev_sampled_token_ids(
        ...
        # The first PP stage only needs the token that starts the next target
        # forward.  Without speculative decoding, sampled_token_ids already
        # has shape [num_reqs, 1].  With speculative decoding it has shape
        # [num_reqs, num_spec_tokens + 1], is left-packed, and uses -1 (or an
        # out-of-vocabulary value) for rejected slots.  Broadcast the last
        # valid token from each row while preserving the existing
        # [num_reqs, 1] receive protocol.
        if sampled_token_ids.shape[-1] > 1:
            valid_mask = (sampled_token_ids >= 0) & (
                sampled_token_ids < self.input_batch.vocab_size
            )
            last_valid_indices = valid_mask.sum(dim=1, keepdim=True) - 1
            sampled_token_ids = sampled_token_ids.gather(
                1, last_valid_indices.clamp_min_(0)
            )
    ...
    def load_model(self, load_dummy_weights: bool = False) -> None:
        ...
        if (
            ...
            drafter = getattr(self, "drafter", None)
            eager_glm53_mtp = (
                os.getenv("VLLM_GLM53_EAGER_MTP", "0") == "1"
                and isinstance(drafter, Glm5NextMTPProposer)
            )
            if (
                drafter is not None
                and hasattr(drafter, "model")
                and not eager_glm53_mtp
            ):
    ...
    def _dummy_run(
        ...
        with self.maybe_dummy_run_with_lora(
            ...
            if self.drafter is not None and self.speculative_config and (
