...
class MambaHybridModelState(DefaultModelState):
    ...
    def __init__(
        ...
        if self._align_mode:
            ...
            self._mamba_group_signature: tuple[tuple[str, ...], ...] = ()
    ...
    def _get_mamba_group_info(
        ...
        group_signature: list[tuple[str, ...]] = []
        for i, group in enumerate(kv_cache_config.kv_cache_groups):
            spec = group.kv_cache_spec
            if isinstance(spec, MambaSpec):
                group_ids.append(i)
                specs.append(spec)
                group_signature.append(tuple(group.layer_names))
        assert specs, "no mamba layers in the model"
        assert all(specs[0] == s for s in specs)

        signature = tuple(group_signature)
        if signature != self._mamba_group_signature:
            # Memory profiling and serving can use different PP-projected KV
            # layouts. Rebuild pointer metadata when the local groups change.
            self._mamba_ctx = None
            self._mamba_group_ids = group_ids
            self._mamba_spec = specs[0]
            self._mamba_group_signature = signature
