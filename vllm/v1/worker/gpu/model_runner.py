...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    @staticmethod
    def _copy_or_reuse_pp_tensor(
        graph_tensor: torch.Tensor, received_tensor: torch.Tensor
    ) -> torch.Tensor:
        """Avoid a D2D copy when PP received into the graph input view."""
        if (
            graph_tensor.data_ptr() == received_tensor.data_ptr()
            and graph_tensor.shape == received_tensor.shape
            and graph_tensor.stride() == received_tensor.stride()
            and graph_tensor.dtype == received_tensor.dtype
            and graph_tensor.device == received_tensor.device
        ):
            return graph_tensor
        # With the fixed-width PP wire, a direct receive may target the
        # leading rows of the graph buffer while the graph itself is padded to
        # a larger capture size.  The received prefix is already in place;
        # clear only the padded tail so stale rows from the previous iteration
        # cannot leak into the graph.
        if (
            graph_tensor.device == received_tensor.device
            and graph_tensor.dtype == received_tensor.dtype
            and graph_tensor.ndim > 0
            and received_tensor.ndim == graph_tensor.ndim
            and graph_tensor.data_ptr() == received_tensor.data_ptr()
            and graph_tensor.shape[1:] == received_tensor.shape[1:]
            and received_tensor.shape[0] <= graph_tensor.shape[0]
        ):
            rows = received_tensor.shape[0]
            if rows < graph_tensor.shape[0]:
                graph_tensor[rows:].zero_()
            return graph_tensor
        return graph_tensor.copy_(received_tensor)
    ...
    def execute_model(
        ...
        if not self.is_first_pp_rank:
            ...
            new_tensors = {
                k: v[:n]
                if dummy_run
                else self._copy_or_reuse_pp_tensor(
                    v[:n], intermediate_tensors.tensors[k][:n]
                )
    ...
    def sample_tokens(
        ...
        current_draft_tokens = None
        if self.num_speculative_steps > 0:
            # Spec-decode and diffusion LLMs both use draft tokens but the latter does
            # not have a speculator (i.e. self.speculator is None)
            current_draft_tokens = self.req_states.draft_tokens[
                input_batch.idx_mapping
            ]
        ...
        if self.pp_handler is not None:
            # Non-last stages need both accepted target tokens and the next
            # proposal before their corresponding pipeline slot reaches decode.
