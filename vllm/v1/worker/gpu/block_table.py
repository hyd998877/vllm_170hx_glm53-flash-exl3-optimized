...
def _compute_slot_mappings_kernel(
    ...
    num_blocks = tl.load(
        num_blocks_ptr + group_id * num_blocks_stride + req_state_idx
    )
    ...
    for i in range(start_idx, end_idx, TRITON_BLOCK_SIZE):
        ...
        # Storage-only hybrid groups can own fewer blocks than the request's
        # token positions imply. Never dereference an unallocated or out-of-row
        # block-table entry; metadata builders for those groups derive their
        # actual storage mapping separately.
        valid_block_indices = (block_indices >= 0) & (
            block_indices < num_blocks
        ) & (
            block_indices < block_table_stride
        )
        is_valid = is_local & valid_block_indices
        safe_block_indices = tl.where(valid_block_indices, block_indices, 0)
        block_numbers = tl.load(
            block_table_ptr + req_state_idx * block_table_stride + safe_block_indices,
            mask=is_valid,
            other=0,
        )
