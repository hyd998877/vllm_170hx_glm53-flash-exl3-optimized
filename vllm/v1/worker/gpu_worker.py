...
class Worker(WorkerBase):
    def __init__(
        ...
        self._pp_fixed_decode_comm = envs.VLLM_PP_FIXED_DECODE_COMM
        self._pp_direct_recv_buffer = envs.VLLM_PP_DIRECT_RECV_BUFFER
        if self._pp_direct_recv_buffer and not self._pp_fixed_decode_comm:
            raise ValueError(
                "VLLM_PP_DIRECT_RECV_BUFFER requires "
                "VLLM_PP_FIXED_DECODE_COMM=1"
            )
    ...
    def execute_model(
        ...
        fixed_decode_tokens = self._get_pp_fixed_decode_tokens(scheduler_output)
        ...
        if forward_pass and not get_pp_group().is_first_rank:
            if fixed_decode_tokens is None:
                ...
                assert self.model_runner.intermediate_tensors is not None
                # Static P2P has no metadata with which to associate a receive
                # operation with its tensor.  Use the same canonical key order
                # on every sender and receiver, including PP0 (which does not
                # own a persistent receive buffer).
                template_tensors = self.model_runner.intermediate_tensors.tensors
                graph_tensors = {
                    key: template_tensors[key][:fixed_decode_tokens]
                    for key in sorted(template_tensors)
                }
                recv_tensors = (
                    graph_tensors
                    if self._pp_direct_recv_buffer
                    else {
                        key: torch.empty_like(tensor)
                        for key, tensor in graph_tensors.items()
                    }
                )
                tensor_dict, comm_handles, comm_postprocess = (
                    get_pp_group().irecv_tensor_dict_static(recv_tensors)
                )
        ...
        if fixed_decode_tokens is None:
            ...
            static_output = self._prepare_pp_fixed_decode_output(
                output.tensors, fixed_decode_tokens
            )
            self._pp_send_work = get_pp_group().isend_tensor_dict_static(static_output)

        return None

    def _prepare_pp_fixed_decode_output(
        self,
        output_tensors: dict[str, torch.Tensor],
        fixed_decode_tokens: int,
    ) -> dict[str, torch.Tensor]:
        """Validate and canonically order a metadata-free PP transfer.

        Non-first PP ranks have a persistent receive-buffer template and can
        validate the complete shape.  PP0 intentionally has no such buffer;
        there we validate the fixed row count and model dtype directly from
        the produced tensors.  Sorting keys on both paths guarantees that the
        metadata-free NCCL operations are posted in the same order.
        """
        intermediate_tensors = self.model_runner.intermediate_tensors
        if intermediate_tensors is not None:
            expected_tensors = intermediate_tensors.tensors
            if set(output_tensors) != set(expected_tensors):
                raise RuntimeError(
                    "Fixed PP decode intermediate keys mismatch: expected "
                    f"{tuple(sorted(expected_tensors))}, got "
                    f"{tuple(sorted(output_tensors))}"
                )
            for key, tensor in output_tensors.items():
                expected = expected_tensors[key][:fixed_decode_tokens]
                if (
                    tensor.ndim == 0
                    or tensor.shape[0] < fixed_decode_tokens
                    or tensor.shape[1:] != expected.shape[1:]
                    or tensor.dtype != expected.dtype
                ):
                    raise RuntimeError(
                        "Fixed PP decode tensor mismatch for "
                        f"{key}: expected prefix shape={expected.shape}, "
                        f"dtype={expected.dtype}; got shape={tensor.shape}, "
                        f"dtype={tensor.dtype}"
                    )
        else:
            # PP0 never receives intermediate tensors, so it has no persistent
            # template.  Its actual model output is the authoritative shape.
            expected_dtype = self.model_runner.model_config.dtype
            for key, tensor in output_tensors.items():
                if (
                    ...
                    raise RuntimeError(
                        "Fixed PP decode tensor mismatch for PP0 "
                        f"{key}: expected first_dim={fixed_decode_tokens}, "
                        f"dtype={expected_dtype}; got shape={shape}, dtype={dtype}"
                    )

        # CUDA graph execution may pad the model output (e.g. 18 rows for a
        # 12-row request).  The static wire carries only the scheduled prefix;
        # sending padded rows would desynchronise the receiver's fixed buffer.
        return {
            key: output_tensors[key][:fixed_decode_tokens]
            for key in sorted(output_tensors)
        }

    def _get_pp_fixed_decode_tokens(
        self, scheduler_output: "SchedulerOutput"
    ) -> int | None:
        """Return a fixed PP wire row count for uniform decode steps.

        DFlash always schedules ``decode_query_len`` rows per request
        (one target token plus its speculative slots).  It is therefore safe
        to use the same static hidden-state wire as regular one-token decode
        once prefill/new-request work has drained.  Other speculative methods
        may have variable query lengths and remain on the metadata path.
        """
        speculative_config = self.vllm_config.speculative_config
        if speculative_config is None:
            expected_tokens_per_req = 1
        elif speculative_config.use_dflash():
            expected_tokens_per_req = getattr(
                self.model_runner, "decode_query_len", None
            )
            if expected_tokens_per_req is None:
                return None
        else:
            return None

        if (
            not self._pp_fixed_decode_comm
        ...
        num_outputs = dict(zip(cached.req_ids, cached.num_output_tokens))
        if any(
            num_tokens != expected_tokens_per_req or num_outputs.get(req_id, 0) <= 0
            for req_id, num_tokens in scheduler_output.num_scheduled_tokens.items()
        ):
            return None

        total = scheduler_output.total_num_scheduled_tokens
        expected_total = len(scheduler_output.num_scheduled_tokens) * int(
            expected_tokens_per_req
        )
        if total != expected_total:
            raise RuntimeError(
                "Fixed PP decode scheduler total mismatch: "
                f"{total} != {expected_total}"
            )
        logger.info_once(
            "Fixed PP decode communication engaged "
            "(metadata=static, direct_recv=%s)",
            self._pp_direct_recv_buffer,
        )
        return total
