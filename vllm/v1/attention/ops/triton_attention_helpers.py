...
def compute_tile_loop_bounds(
    ...
    tile_start = 0
    tile_end = num_tiles
    # DFlash's non-causal sliding attention is block anchored rather than a
    # query-centred bidirectional window. The query suffix is one draft block:
    # every row sees the entire block and the same trailing history window.
    if SLIDING_WINDOW > 0 and not USE_MM_PREFIX:
        if USE_DFLASH_BLOCK:
            first_allowed_key = context_len - SLIDING_WINDOW
...
def compute_kv_seq_mask(
    ...
    if CHUNK_LOOKBACK > -1:
        ...
        if USE_DFLASH_BLOCK:
            # Exactly ``SLIDING_WINDOW`` history tokens plus the full current
            # draft block [context_len, seq_len), shared by every query row.
            history_start = dflash_context_len - SLIDING_WINDOW
            seq_mask = (seq_offset[None, :] >= history_start) & (
                seq_offset[None, :] < seq_len
            )
