...
class Glm5NextMTPProposer(Step3p5MTPProposer):
    """GLM-5.3 MTP proposer with separate MLA/indexer cache groups.

    GLM-5.3's draft layer contains both sparse MLA and its kpool indexer, so
    the generic one-group MTP proposer cannot build correct metadata for it.
    Reuse the multi-group machinery above while retaining the generic MTP
    lm-head sharing required by GLM-5.3.
    """
