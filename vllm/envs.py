...
if TYPE_CHECKING:
    ...
    VLLM_PP_FIXED_DECODE_COMM: bool = False
    VLLM_PP_DIRECT_RECV_BUFFER: bool = False
    VLLM_PP_DECODE_PHASE_POLICY: Literal["native", "pairpack", "pack"] = "native"
    ...
    VLLM_EXL3_MARLIN_LAYERS: str | None = None
    VLLM_EXL3_NON_ROUTED_FP8: bool = False
    VLLM_EXL3_NON_ROUTED_FP8_SCOPE: Literal["all", "official"] = "all"
    VLLM_EXL3_OUTER_GRAPH: bool = False
    VLLM_EXL3_SHARED_INPUT_HAD: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Pure non-speculative decode has a worker-reproducible tensor shape. Skip
    # the per-step CPU metadata exchange on that exact hot path.
    "VLLM_PP_FIXED_DECODE_COMM": lambda: bool(
        int(os.getenv("VLLM_PP_FIXED_DECODE_COMM", "0"))
    ),
    # Receive fixed decode tensors into the persistent CUDA graph input view,
    # avoiding a second device-to-device copy.
    "VLLM_PP_DIRECT_RECV_BUFFER": lambda: bool(
        int(os.getenv("VLLM_PP_DIRECT_RECV_BUFFER", "0"))
    ),
    # Group asynchronous PP decode requests into stable two-request phases.
    # This avoids six independent one-request phases for the target C6 profile.
    "VLLM_PP_DECODE_PHASE_POLICY": lambda: os.getenv(
        "VLLM_PP_DECODE_PHASE_POLICY", "native"
    ),
    "VLLM_PP_PREFILL_COHORT_BARRIER": lambda: bool(
        int(os.getenv("VLLM_PP_PREFILL_COHORT_BARRIER", "0"))
    ),
    "VLLM_PP_PREFILL_COHORT_SIZE": lambda: int(
        os.getenv("VLLM_PP_PREFILL_COHORT_SIZE", "0")
    ),
    ...
    "VLLM_EXL3_MARLIN_DIR": lambda: os.getenv("VLLM_EXL3_MARLIN_DIR"),
    "VLLM_EXL3_MARLIN_LAYERS": lambda: os.getenv("VLLM_EXL3_MARLIN_LAYERS"),
    "VLLM_EXL3_NON_ROUTED_FP8": lambda: bool(
        int(os.getenv("VLLM_EXL3_NON_ROUTED_FP8", "0"))
    ),
    "VLLM_EXL3_NON_ROUTED_FP8_SCOPE": lambda: os.getenv(
        "VLLM_EXL3_NON_ROUTED_FP8_SCOPE", "all"
    ),
    "VLLM_EXL3_OUTER_GRAPH": lambda: bool(
        int(os.getenv("VLLM_EXL3_OUTER_GRAPH", "0"))
    ),
    "VLLM_EXL3_SHARED_INPUT_HAD": lambda: bool(
        int(os.getenv("VLLM_EXL3_SHARED_INPUT_HAD", "0"))
    ),
