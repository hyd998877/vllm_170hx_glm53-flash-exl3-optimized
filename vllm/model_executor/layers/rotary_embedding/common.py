...
class ApplyRotaryEmb(CustomOp):
    ...
    def forward_cuda(
        ...
        # Some lightweight/runtime builds ship the flash-attention extension
        # without the optional ``layers.rotary`` Python shim.  Vision encoder
        # profiling still needs a correct RoPE path in that configuration;
        # use the native implementation rather than failing engine startup.
        try:
