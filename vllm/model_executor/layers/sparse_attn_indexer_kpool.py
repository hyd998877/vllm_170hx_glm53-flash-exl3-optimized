...
def _torch_mqa_logits(
    ...
    """Small-hardware fallback for DeepGEMM MQA logits.

    DeepGEMM is intentionally Hopper/Blackwell-only.  SM80 can still execute
    the indexer correctly by decoding the FP8 workspace to BF16 and reducing
    one head at a time.  The implementation keeps only an ``M x N`` scratch
    tensor (rather than materialising ``M x H x N``), which is important for
    long-context chunked prefills.
    """
    ...
    if k_scales is not None:
        kf = kf * k_scales.reshape(-1, 1).float()
    out = torch.zeros(
        (q.shape[0], k.shape[0]), dtype=torch.float32, device=q.device
    )
    for head in range(q.shape[1]):
        scores = torch.mm(qf[:, head, :], kf.transpose(0, 1))
        scores = scores.clamp_min_(0.0)
        out.add_(scores * weights[:, head : head + 1].float())
...
def _torch_paged_mqa_logits(
    ...
    """SM80 fallback for paged indexer logits.

    ``kv_cache`` is the byte view used by the native indexer cache: FP8 K
    values followed by one FP32 scale per token.  Decode batches are small, so
    gathering the live rows before the MQA reduction is preferable to a large
    custom kernel that cannot use SM80 FP8 conversion instructions.
    """
    if q.dtype == torch.float8_e4m3fn:
        q = q.to(torch.bfloat16)
    raw = kv_cache.squeeze(-2) if kv_cache.ndim == 4 else kv_cache
    head_dim = q.shape[-1]
    ...
    for b in range(batch):
        length = min(int(context_lens[b].max().item()), max_model_len)
        if length <= 0:
            continue
        pos = torch.arange(length, device=q.device, dtype=torch.long)
        page = pos // raw.shape[1]
        off = pos % raw.shape[1]
        block_ids = block_tables[b, page].to(torch.long)
        rows = raw[block_ids, off]
        k = rows[:, :head_dim].contiguous().view(torch.float8_e4m3fn)
        scales = rows[:, head_dim:].contiguous().view(torch.float32).reshape(-1)
        for n in range(next_n):
            row = b * next_n + n
            out[row, :length] = _torch_mqa_logits(
                q[b, n], k, weights[row], scales
            )
            q_pos = length - next_n + n
            if q_pos + 1 < length:
                out[row, q_pos + 1 : length] = float("-inf")
    return out
...
def sparse_attn_indexer_kpool(
    ...
    if not isinstance(attn_metadata, dict):
        ...
        decode_logits_elems = worst_decode_tokens * max_model_len
        prefill_logits_elems = (
            envs.VLLM_SPARSE_INDEXER_MAX_LOGITS_MB * 1024 * 1024 // 4
        )
        logits_spec = (
            (max(decode_logits_elems, prefill_logits_elems),),
            torch.float32,
        )
    ...
    if has_prefill:
        ...
        logits_elems = envs.VLLM_SPARSE_INDEXER_MAX_LOGITS_MB * 1024 * 1024 // 4
        k_quant_full, k_scale_full, logits_workspace = (
            workspace_manager.get_simultaneous(
            values_spec,
            scales_spec,
                ((logits_elems,), torch.float32),
            )
        )
        for chunk in prefill_metadata.chunks if not short_prefill else ():
            ...
            if current_platform.is_rocm():
                ...
                if use_fp4_cache:
                    raise RuntimeError("SM80 indexer fallback does not support FP4")
...
class SparseAttnIndexerKpool(CustomOp):
    ...
    def __init__(
        ...
        if use_fp4_cache and current_platform.is_cuda() and not has_deep_gemm():
            raise RuntimeError("FP4 sparse indexer cache requires DeepGEMM.")
