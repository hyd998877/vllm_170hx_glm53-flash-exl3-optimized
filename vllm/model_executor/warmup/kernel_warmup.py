...
from vllm.model_executor.warmup.glm5_next_warmup import glm5_next_warmup
...
def kernel_warmup(worker: "Worker", *, process_local_only: bool = False):
    ...
    # GLM-5.3's sparse indexer/mHC path dispatches several Triton/TileLang
    # kernels lazily from custom ops, so the generic registry cannot observe
    # their concrete shapes.  Prime them before serving the first request.
    glm5_next_warmup(
        worker.get_model(),
