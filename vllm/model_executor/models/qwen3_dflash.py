...
def dflash_target_rope_is_neox_style(target_model: nn.Module) -> bool | None:
    """The target's primary-attention RoPE layout.
    ...
    from the target. None if the target uses no primary-attention RoPE.

    Do not scan arbitrary rotary modules here. Sparse-MLA models can have no
    RoPE in their primary attention while still owning an interleaved RoPE for
    an auxiliary sparse indexer. Treating that indexer as the target layout
    silently changes a Qwen-family DFlash head from its NeoX default to GPT-J
    layout. Only modules installed as ``rotary_emb`` are primary-attention
    candidates; names such as ``indexer_rope_emb`` are deliberately ignored.
    """
    language_model = (
    ...
    auxiliary: list[tuple[str, bool]] = []
    for name, module in language_model.named_modules():
        style = getattr(module, "is_neox_style", None)
        if not isinstance(style, bool):
            continue
        if name.rsplit(".", 1)[-1] == "rotary_emb":
            logger.info(
                "Resolved DFlash RoPE layout from target primary attention "
                "module %s: is_neox_style=%s",
                name,
                style,
            )
            return style
        auxiliary.append((name, style))
    if auxiliary:
        logger.info(
            "Target has no primary-attention rotary_emb; ignoring auxiliary "
            "RoPE modules for DFlash layout: %s",
            auxiliary,
        )
    return None
...
class DFlashQwen3Attention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            dflash_block_attention=(not causal and sliding_window is not None),
        )
        self.causal = causal
...
class DFlashQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # A DFlash head is hosted on the last worker of a PP target but is
        # itself a single local model.  Do not apply the target's explicit
        # VLLM_PP_LAYER_PARTITION (which has one entry per target stage) when
        # deriving the layer-name offset for the draft head.
