...
def test_prefill_cohort_last_waiting_member_makes_progress():
    """The member completing a waiting cohort must not be deferred."""
    scheduler = object.__new__(AsyncScheduler)
    scheduler._pp_prefill_cohort_barrier = True
    scheduler._pp_prefill_cohort_ids = {"0", "1"}
    scheduler._pp_prefill_cohort_ready = set()
    scheduler._pp_prefill_cohort_held = set()
    scheduler._pp_prefill_cohort_final_pending = set()
    scheduler._pp_prefill_cohort_finalizing = False
    scheduler._pp_prefill_cohort_released = False
    scheduler._pp_prefill_final_chunk = 4
    scheduler._pp_prefill_cohort_min_tokens = 8
    requests = create_requests(num_requests=2, num_tokens=8)

    assert scheduler._should_defer_waiting_prefill(requests[0], 4)
    assert not scheduler._should_defer_waiting_prefill(requests[1], 4)
    assert scheduler._pp_prefill_cohort_finalizing
    assert not scheduler._pp_prefill_cohort_held


def test_prefill_cohort_waits_for_final_chunk_output():
    """Do not release decode while final prefill chunks are in flight."""
    scheduler = object.__new__(AsyncScheduler)
    scheduler._pp_prefill_cohort_barrier = True
    scheduler._pp_prefill_cohort_ids = {"0", "1"}
    scheduler._pp_prefill_cohort_ready = {"0", "1"}
    scheduler._pp_prefill_cohort_held = set()
    scheduler._pp_prefill_cohort_final_pending = {"0", "1"}
    scheduler._pp_prefill_cohort_finalizing = True
    scheduler._pp_prefill_cohort_released = False
    scheduler._pp_prefill_final_chunk = 4
    scheduler._pp_prefill_cohort_min_tokens = 8
    requests = create_requests(num_requests=2, num_tokens=8)
    ...
    scheduler._update_prefill_cohort_barrier()
    assert not scheduler._pp_prefill_cohort_released
    scheduler._pp_prefill_cohort_final_pending.clear()
    scheduler._update_prefill_cohort_barrier()
    assert scheduler._pp_prefill_cohort_released


def test_prefill_cohort_releases_when_kv_capacity_stalls():
    """A waiting cohort member must not deadlock held resident members."""
    scheduler = object.__new__(AsyncScheduler)
    scheduler._pp_prefill_cohort_barrier = True
    scheduler._pp_prefill_cohort_ids = {"0", "1"}
    scheduler._pp_prefill_cohort_held = {"0"}
    scheduler._pp_prefill_cohort_finalizing = False
    scheduler._pp_prefill_cohort_released = False
    ...
    assert scheduler._should_release_stalled_prefill_cohort(
