...
def test_glm5_dflash_draft_keeps_separate_kv_groups():
    """DFlash SWA pages must not flatten GLM's MLA/indexer/Mamba layout."""
    specs = _glm5_like_kv_cache_spec_with_tail()
    for i in range(5):
        specs[f"draft.layers.{i}.attn"] = new_sliding_window_spec(
            block_size=1024,
            num_kv_heads=8,
            head_size=128,
            dtype=torch.bfloat16,
    ...
    config = SimpleNamespace(
        ...
        model_config=SimpleNamespace(get_total_num_hidden_layers=lambda: 45),
    ...
    bytes_per_block = kv_cache_utils._pool_bytes_per_block(groups)
    kv_cache_config = kv_cache_utils.get_kv_cache_config_from_groups(
        config, groups, bytes_per_block * 100
    )
    tensors = _tensor_by_layer(kv_cache_config)
