...
def test_slot_mapping_pads_positions_beyond_allocated_blocks():
    """Storage-only hybrid groups can have shorter tables than token positions."""
    device = torch.device("cuda")
    block_tables = BlockTables(
        block_sizes=[16],
        max_num_reqs=1,
    ...
    actual = block_tables.compute_slot_mappings(
        ...
        positions=torch.tensor([0, 15, 16, 63], dtype=torch.int64, device=device),
        num_tokens_padded=4,
    )[0]

    assert torch.equal(
        actual,
        torch.tensor([48, 63, -1, -1], dtype=torch.int64, device=device),
    )
