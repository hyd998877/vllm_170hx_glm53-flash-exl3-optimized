...
def test_builder_reuses_slot_mapping_storage_between_steps():
    builder = make_tail_builder()
    first = make_common_metadata([list(range(8))], [5])
    second = make_common_metadata([list(range(8, 16))], [9])
    ...
    assert torch.equal(
        second_meta.slot_mapping[:8],
        torch.tensor([36, 37, 38, 39, 36, 37, 38, 39]),
    )


def test_tail_mapping_can_reuse_a_persistent_output_buffer():
    first = make_common_metadata([list(range(8))], [5])
    second = make_common_metadata([list(range(8, 16))], [9])
    persistent = torch.empty_like(first.slot_mapping)

    first_out = compute_kpool_tail_slot_mapping(
        first.slot_mapping,
        first.block_table_tensor,
        first.query_start_loc,
        first.positions,
        first.num_actual_tokens,
        first.num_reqs,
        KPOOL,
        out=persistent,
    )
    second_out = compute_kpool_tail_slot_mapping(
        second.slot_mapping,
        second.block_table_tensor,
        second.query_start_loc,
        second.positions,
        second.num_actual_tokens,
        second.num_reqs,
        KPOOL,
        out=persistent,
    )

    assert first_out.data_ptr() == second_out.data_ptr() == persistent.data_ptr()
    assert torch.equal(
        second_out[:8], torch.tensor([36, 37, 38, 39, 36, 37, 38, 39])
    )
    assert torch.equal(second_out[8:], torch.full_like(second_out[8:], -1))
