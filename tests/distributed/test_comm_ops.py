...
def test_isend_tensor_dict_device_tensors_use_pair_group(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    direct_calls: list[tuple[torch.Tensor, Any]] = []
    batches: list[list[Any]] = []

    def fake_isend(t: torch.Tensor, *args: Any, **kwargs: Any) -> _DummyWork:
        direct_calls.append((t, kwargs.get("group")))
        return _DummyWork()
    ...
    def fake_batch(ops: list[Any]) -> list[_DummyWork]:
        batches.append(ops)
        return [_DummyWork() for _ in ops]

    monkeypatch.setattr(torch.distributed, "batch_isend_irecv", fake_batch)
    g = _make_group_for_unit_test(rank_in_group=0, world_size=2)
    device_pair = object()
    g._pp_p2p_device_groups = {(0, 1): device_pair}  # type: ignore[assignment]
    g.send_object = lambda obj, dst: None  # type: ignore[method-assign]
    tensors = {
        "device_a": torch.empty(2, device="meta"),
        "cpu": torch.empty(3),
        "device_b": torch.empty(4, device="meta"),
    }

    handles = g.isend_tensor_dict(tensors, dst=1)

    assert direct_calls == [(tensors["cpu"], g.cpu_group)]
    assert len(batches) == 1
    assert [op[1] for op in batches[0]] == [
        tensors["device_a"],
        tensors["device_b"],
    ]
    assert all(op[3] is device_pair for op in batches[0])
    assert len(handles) == 3


def test_irecv_tensor_dict_device_tensors_use_pair_group(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    direct_calls: list[tuple[torch.Tensor, Any]] = []
    batches: list[list[Any]] = []
    ...
    g._pp_p2p_device_groups = {(0, 1): device_pair}  # type: ignore[assignment]
    metadata = [
        ("device_a", TensorMetadata("meta", torch.float32, torch.Size([2]))),
        ("cpu", TensorMetadata("cpu", torch.float32, torch.Size([3]))),
        ("device_b", TensorMetadata("meta", torch.float32, torch.Size([4]))),
    ]
    g.recv_object = lambda src: metadata  # type: ignore[method-assign]

    tensor_dict, handles, postprocess = g.irecv_tensor_dict(src=0)

    assert tensor_dict is not None
    assert list(tensor_dict) == ["device_a", "cpu", "device_b"]
    assert direct_calls == [(tensor_dict["cpu"], g.cpu_group)]
    assert len(batches) == 1
    assert [op[1] for op in batches[0]] == [
        tensor_dict["device_a"],
        tensor_dict["device_b"],
    ]
    assert all(op[3] is device_pair for op in batches[0])
    assert len(handles) == 3
    assert postprocess == []
...
def test_static_tensor_dict_comm_skips_metadata_and_uses_pair_group(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[tuple[str, torch.Tensor, Any]] = []

    def fake_isend(t: torch.Tensor, *args: Any, **kwargs: Any) -> _DummyWork:
        calls.append(("send", t, kwargs.get("group")))
        return _DummyWork()

    def fake_irecv(t: torch.Tensor, *args: Any, **kwargs: Any) -> _DummyWork:
        calls.append(("recv", t, kwargs.get("group")))
        return _DummyWork()
    ...
    device_pair = object()
    sender._pp_p2p_device_groups = {(0, 1): device_pair}  # type: ignore[assignment]
    receiver._pp_p2p_device_groups = {  # type: ignore[assignment]
        (0, 1): device_pair
    }
    sender.send_object = Mock(side_effect=AssertionError("metadata send used"))
    receiver.recv_object = Mock(side_effect=AssertionError("metadata recv used"))

    source = torch.arange(8, dtype=torch.float32, device="meta")
    target = torch.empty(8, device="meta")
    send_handles = sender.isend_tensor_dict_static({"hidden_states": source})
    tensor_dict, recv_handles, postprocess = receiver.irecv_tensor_dict_static(
        {"hidden_states": target}
    )

    assert calls == [
        ("send", source, device_pair),
        ("recv", target, device_pair),
    ]
    assert tensor_dict["hidden_states"] is target
    assert len(send_handles) == len(recv_handles) == 1
    assert postprocess == []
